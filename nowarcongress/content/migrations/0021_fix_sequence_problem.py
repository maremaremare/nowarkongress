...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        if orm.OuterParticipant.objects.count():
            highest_number = db.execute('select id from content_outerparticipant order by id desc limit 1;')[0][0]
            db.execute('alter sequence content_outerparticipant_id_seq restart with %s;' % (highest_number + 1))
